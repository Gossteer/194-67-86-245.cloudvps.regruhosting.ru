...
class RequestController extends Controller
{
    ...
    {
        return response()->json($travel_payouts_services->searchResults($travel_payouts_services->searchTickets($request), 10, 10));
    }
    ...
    public function sendFirstSearchTickets(Request $request, VkApi $vk_api, FormationMessageServices $formation_message_services): JsonResponse
    {
        ...
        foreach ($messages as $message) {
            $response[] = $vk_api->messagesSend(['user_id' => $request->user_id], $message['message'], config('vk.groups.SEND_FIRST_SEARCH_VK_PUBLIC_ID', '204613902'), true, $message['keyboard']);
        }
